...
// XMR/USD candles, BTC ratio, privacy-coin peer group, top-10 normalized,
// exchange volume, DEX/atomic-swap volume, order book depth (mock data).
//
// All numerics here are SIMULATED until claude.ai's live feed is wired.
// To swap in real data: replace the `MARKETS_*` consts and any inline
// generators with reads from window.useMarketsLive() — exposed from
// shared.jsx in the same shape.

function genCandles(days, start, vol) {
...
function CandleChart({ candles, width = 760, height = 240, color = "var(--tk-accent)" }) {
...
function MultiLine({ series, width = 760, height = 240, labels = true }) {
...
function MarketsPage({ data, bg }) {
  ...
  const peerSeries = React.useMemo(() => [
    ...
    <AppShell active="markets" data={data} bg={{ intensity: "calm" }}>
      ...
      <PanelFrame title={`Order book depth · ${data.price.toFixed(2)} XMR/USD`} right={<span>mid: ${data.price.toFixed(2)} · ±1.6%</span>}>
        ...
        <p className="mono dim" style={{ marginTop: 12, fontSize: 10.5, color: "var(--ink-40)" }}>
          ⓘ Aggregated mock depth · plug a real exchange websocket via shared.jsx · useMoneroLive() to live this.
        </p>
      </PanelFrame>
    </AppShell>
  );
