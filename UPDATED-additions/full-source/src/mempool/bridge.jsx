...
function BrgCard({ title, right, children, pad = "14px 16px", style }) {
  return (
...
function BrgGauge({ value, label, unit = "%", color = "var(--tk-accent)", size = 132 }) {
  ...
  const ax = (ang, rad) => c + Math.cos(ang) * rad;
  const ay = (ang, rad) => cyy + -Math.sin(ang) * rad;
  const arc = (from, to, rad) => `M ${ax(from, rad)} ${ay(from, rad)} A ${rad} ${rad} 0 0 1 ${ax(to, rad)} ${ay(to, rad)}`;
...
function BrgAlertTape({ data }) {
  ...
  function mk(s) { const d = new Date(); return { lvl: s[0], tone: s[1], msg: s[2], ts: d.toISOString().slice(11, 19) }; }
  React.useEffect(() => {
    let n = 100;
  ...
  const col = { g: "var(--g-50)", acc: "var(--tk-accent)", p: "var(--p-50)", y: "var(--y-50)" };
  return (
...
function BrgTxConsole({ data, onPickTx }) {
  const rows = data.mempool.slice(0, 12);
  const phase = (i) => i % 4 === 0 ? ["STEM", "var(--p-50)"] : i % 4 === 1 ? ["QUEUE", "var(--y-50)"] : i % 4 === 2 ? ["FLUFF", "var(--tk-accent)"] : ["POOL", "var(--c-50)"];
  return (
...
function BrgOverview({ data, onPickTx }) {
...
function BridgeView({ data, bg }) {
  const MempoolSearchBar = window.MempoolSearchBar;
  const TrackingDetail = window.TrackingDetail;
  const { tracking, onSearch, clearTracking } = window.useMempoolTracking(data);
  return (
      ...
      <div className="art-stage">
        ...
        <div className="shell">
          ...
          <div className="main" style={{ overflow: "auto", padding: 0 }}>
            {tracking ? (
              <TrackingDetail tracking={tracking} data={data} onBack={clearTracking} onPickTx={(id) => onSearch({ kind: "tx", id })} />
            ) : (
