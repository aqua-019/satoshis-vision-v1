// metaphors.jsx — 8 metaphor-driven visualizations for Education.
// Each export uses ProtoArtboard chrome (stage + side panel).
//
// Exports to window:
//   HearthView      · Tail emission · the eternal hearth
//   MetronomeView   · Block target · the 2-min metronome
//   SiloView        · Monetary policy · grain silo + faucet (BTC vs XMR)
//   ThermostatView  · Difficulty adjustment · two needles tracking
//   AuctionView     · Mempool fees · bidding paddles
//   SkylineView     · Pool decentralization · city skyline + HHI
//   BloodhoundView  · Privacy attacks · hound losing the trail
//   BalanceView     · Confidential amounts · envelope-on-balance
...
function MetronomeView({ data, bg }) {
  return (
    <ProtoArtboard
      label="M2 · Metronome"
...
window.MetronomeView = MetronomeView;
...
function SiloStage() {
  ...
  const SiloViz = ({ x, supply, max, faucet, label, color, drip }) => (
...
function ThermostatStage() {
  ...
  const dial = (cx, cy, label, val, units, color, range, animLag) => {
...
function ThermostatView({ data, bg }) {
  return (
    <ProtoArtboard
      label="M4 · Thermostat"
...
function AuctionStage({ mempool, height }) {
  const tick = useTick(80);
    ...
    <ProtoArtboard
      ...
      stage={<><AuctionStage mempool={data.mempool} height={data.height} />
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          ...
          <Stat k="TOP FEE" v="~1.0K" sub="piconero / B" tone="y" />
        </div>
      </>}
      panel={
        <>
...
function SkylineStage({ poolDist }) {
    ...
    <ProtoArtboard
      ...
      stage={<><SkylineStage poolDist={data.poolDist} />
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          ...
          <Stat k="P2POOL SHARE" v={(data.poolDist.find(p => p.name === "P2Pool")?.share * 100 || 0).toFixed(1) + "%"} sub="decentralized" tone="g" />
          ...
          <Stat k="HHI" v={Math.round(data.poolDist.reduce((a, p) => a + Math.pow(p.share * 100, 2), 0)).toLocaleString()} sub="concentration" tone="y" />
        </div>
      </>}
      panel={
        <>
...
window.SkylineView = SkylineView;
          ...
          return (
            <g key={i} transform={`translate(${s.x}, 200)`}>
              <rect x="-12" y="0" width="4" height="120" fill={passed ? "rgba(184,122,255,0.5)" : "var(--ink-20)"}
                filter={passed ? "url(#fx-glow)" : null} />
          ...
          {Array.from({ length: 14 }).map((_, i) => {
            ...
            return (
              <circle key={i} cx={x} cy="324" r="2.4"
                ...
                filter={stageAlpha > 0.5 ? "url(#fx-glow)" : null} />
            );
          })}
        </g>
...
function BloodhoundView({ data, bg }) {
  return (
    <ProtoArtboard
      label="M7 · Bloodhound"
      ...
      stage={<><BloodhoundStage />
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          ...
          <Stat k="LARGEST BOUNTY" v="$625K" sub="2020 · IRS" tone="y" />
...
window.BloodhoundView = BloodhoundView;
...
window.BalanceView = BalanceView;
