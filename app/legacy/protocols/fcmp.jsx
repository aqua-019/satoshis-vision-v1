      ...
      {ringPts.map((p, i) => {
        ...
        return (
          <g key={i} opacity={ringOpacity}>
            ...
            <circle cx={p.x} cy={p.y} r="3.6" fill="#fff1e0"
              style={{ filter: "drop-shadow(0 0 8px var(--tk-accent))" }} />
...
function CurveTree({ depth = 4 }) {
  // Cute fractal commitment tree
  const W = 920, H = 220;
  const lines = [];
  const dots = [];
  const recurse = (x, y, len, ang, d) => {
    if (d > depth) return;
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: W, display: "block" }}>
      ...
      {dots.map((d, i) => (
        <circle key={i} cx={d.x} cy={d.y} r={Math.max(1.4, 3.6 - d.d * 0.6)}
          ...
          style={d.d === 0 ? { filter: "drop-shadow(0 0 8px #b87aff)" } : undefined} />
      ))}
