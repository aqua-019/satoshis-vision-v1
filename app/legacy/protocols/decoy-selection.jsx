...
function TimeTide({ ringSize = 16, trueAge = 7, total = 380 }) {
  ...
  const xOf = (age) => padL + (Math.min(age, ageMax) / ageMax) * innerW;
        ...
        return (
          <g key={i}>
            ...
            <circle cx={x} cy={yWave + bob} r={r.real ? 5 : 4}
              fill={r.real ? "#fff1e0" : "rgba(255,180,80,0.95)"}
              stroke={r.real ? "#ffd9a0" : "rgba(255,200,120,0.4)"} strokeWidth="1"
              style={{ filter: r.real ? "drop-shadow(0 0 8px #ff7a1a)" : "drop-shadow(0 0 4px rgba(255,122,26,0.8))" }} />
            {/* label */}
            <text x={x} y={yWave + bob - 12} textAnchor="middle" fontFamily="var(--f-mono)" fontSize="9"
              fill={r.real ? "#fff1e0" : "rgba(255,200,140,0.7)"}
...
function IndistinguishabilityProbe({ ring }) {
  return (
...
function DecoySelectionView({ data, bg }) {
...
window.DecoySelectionView = DecoySelectionView;
