...
function AssemblyStation({ n, title, kind, on, done, tick, w = 200, h = 220 }) {
  const renderInside = () => {
    if (kind === "input") {
            ...
            return (
              <g key={i}>
                <circle cx={cx} cy={cy} r={isReal ? 5 : 3.2}
                  fill={isReal ? "#fff1e0" : "rgba(255,180,80,0.85)"}
                  opacity={flicker}
                  style={{ filter: isReal ? "drop-shadow(0 0 8px var(--tk-accent))" : "drop-shadow(0 0 3px rgba(255,122,26,0.6))" }} />
      ...
      return (
        <g>
          ...
          <circle cx={w / 2} cy={h / 2 - 16} r="34" fill="rgba(255,122,26,0.25)" stroke="var(--tk-accent)" strokeWidth="2"
            style={{ filter: "drop-shadow(0 0 12px var(--tk-accent))" }} />
          <text x={w / 2} y={h / 2 - 18} textAnchor="middle" fontFamily="var(--f-mono)" fontSize="11" fill="#fff" letterSpacing="0.1em" style={{ textShadow: "var(--glow-1)" }}>CLSAG</text>
          <text x={w / 2} y={h / 2 - 4} textAnchor="middle" fontFamily="var(--f-mono)" fontSize="8" fill="rgba(255,255,255,0.85)">σ · 16-ring</text>
          {/* seal rays */}
