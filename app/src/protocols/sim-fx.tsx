// Ported from five01/src/protocols/sim-fx.jsx — shared FX primitives for the
...
//   <Spotlight />          — cursor-following soft light overlay
//
...
export function SvgDefs({ id = "fx" }: any) {
  return (
    <defs>
...
export function GlowOrb({ cx, cy, r = 6, color = "var(--tk-accent)", gradId, pulse = false }: any) {
  const id = "fx";
  return (
...
export function ParticleStream({
  ...
  React.useEffect(() => {
    ...
    // not running the loop at all when nobody can see it matters here more
    // than anywhere else in the app.
    const undrawable = observeDrawable(canvas, (drawable) => {
      if (drawable) { if (!raf) tick(); }
      else if (raf) { cancelAnimationFrame(raf); raf = 0; }
    });

    return () => { undrawable(); if (raf) cancelAnimationFrame(raf); };
  }, [tier, count, width, height, color, altColor, altRatio, speedMul, gravity, size[0], size[1], trail, depth, spread, cx, cy, radius]);
...
export function Volumetric3D({ children, depth = 800, intensity = 1, style, className }: any) {
...
export function Spotlight({ children, color = "rgba(255,122,26,0.08)", radius = 320, className }: any) {
  const ref = React.useRef<HTMLDivElement>(null);
  React.useEffect(() => {
    const el = ref.current;
    if (!el) return;
  ...
  return (
    <div ref={ref} className={"spotlight " + (className || "")}
      style={{ position: "relative", "--sx": "50%", "--sy": "50%" } as React.CSSProperties}>
      {children}
      <div style={{
