      ...
      <div className="v6-future-card">
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            ...
            {stale ? <span className="mono" style={{ fontSize: 10, color: "var(--y-50)" }}>QUIET &gt;90D</span> : null}
          </div>
          ...
          <p className="mono dim" style={{ margin: 0, fontSize: 12.5, lineHeight: 1.6 }}>{p.lede}</p>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
          <span className="mono dim2" style={{ fontSize: 11, letterSpacing: "0.1em" }}>
  ...
  return (
    <div className="panel" style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        <span className="mono dim2" style={{ fontSize: 10.5, letterSpacing: "0.1em", textTransform: "uppercase" }}>{label}</span>
        <span className="mono dim2" style={{ fontSize: 10 }}>{repo}</span>
      </div>
      {pulse ? (
        <div className="mono" style={{ fontSize: 12, color: "var(--ink-80)", display: "flex", gap: 12, alignItems: "center" }}>
        ...
        <div className="mono dim2" style={{ fontSize: 11 }}>fetching via /api/feeds …</div>
      )}
    </div>
  );
}
...
function FeedEmpty({ state, endpoint, what }: { state: FeedState; endpoint: string; what: string }) {
  if (state === "load" || state === "cached") {
    return <div className="mono dim2" style={{ fontSize: 11.5 }}>fetching…</div>;
  }
  return (
    <div className="mono" style={{ fontSize: 11.5, lineHeight: 1.7, color: "var(--y-50)", border: "1px dashed var(--ink-20)", padding: "10px 12px" }}>
  ...
  return (
    <Card style={{ padding: 22 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
        ...
        <span className="mono dim2" style={{ fontSize: 10 }}>getmonero.org · monero-project/research-lab</span>
      </div>
      ...
      <div className="col-2" style={{ gap: 22 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <span className="mono" style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--ink-60)" }}>Announcements</span>
            <span className="mono dim2" style={{ fontSize: 10 }}>{stamp(postAt)}</span>
          </div>
          {posts && posts.length ? posts.map((p) => (
            ...
            >
              ...
              <span className="mono dim2" style={{ fontSize: 10, flexShrink: 0 }}>{(p.date || "").slice(0, 10)}</span>
            </a>
          )) : (
            <FeedEmpty state={postState} endpoint={`${FEED_PROXY}?src=getmonero`} what="getmonero.org's Atom feed" />
          )}
          <a className="mono dim2" href="https://www.getmonero.org/blog/" target="_blank" rel="noopener noreferrer" style={{ fontSize: 10.5, marginTop: 2 }}>all announcements ↗</a>
        </div>

        {/* MRL issue traffic */}
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <span className="mono" style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--ink-60)" }}>MRL · research lab</span>
            <span className="mono dim2" style={{ fontSize: 10 }}>{stamp(issAt)}</span>
          </div>
          {issues && issues.length ? issues.map((i) => (
            ...
            >
              ...
              <span className="mono dim2" style={{ fontSize: 10, flexShrink: 0 }}>{agoStr(i.u)}</span>
            </a>
          )) : (
            <FeedEmpty state={issState} endpoint={`${FEED_PROXY}?src=mrl`} what="the research-lab issue list" />
          )}
          <a className="mono dim2" href="https://github.com/monero-project/research-lab/issues" target="_blank" rel="noopener noreferrer" style={{ fontSize: 10.5, marginTop: 2 }}>all MRL issues ↗</a>
        </div>
      </div>
      ...
      <div style={{ borderTop: "1px solid var(--rule)", marginTop: 16, paddingTop: 12, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <span className="mono dim2" style={{ fontSize: 10.5, letterSpacing: "0.1em", textTransform: "uppercase" }}>Also tracked</span>
