  ...
  return (
    <div className="edu-head" style={{ display: "flex", flexDirection: "column", gap: 10, padding: "8px 0 2px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <span className="mono" style={{ fontSize: "var(--fs-label)", letterSpacing: "0.22em", color: "var(--tk-accent)", textShadow: "var(--glow-1)" }}>{n}</span>
      ...
      {sub ? <p className="mono" style={{ margin: 0, maxWidth: "78ch", fontSize: "var(--fs-body)", lineHeight: 1.7, color: "var(--ink-60)" }}
...
export function EduMilestone({ date, title, children, tone = "var(--tk-accent)" }: EduMilestoneProps) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 24, padding: "18px 0", borderTop: "1px solid var(--rule)" }}>
      <div>
        <div className="mono" style={{ fontSize: "var(--fs-label)", color: tone, letterSpacing: "0.04em", textShadow: tone === "var(--tk-accent)" ? "var(--glow-1)" : "none" }}>{date}</div>
        <div className="serif" style={{ fontSize: 21, fontWeight: 500, color: "var(--ink-100)", marginTop: 6, lineHeight: 1.2 }}>{title}</div>
      </div>
      <div className="mono" style={{ fontSize: "var(--fs-body)", lineHeight: 1.72, color: "var(--ink-80)" }}>{children}</div>
    </div>
  );
}
