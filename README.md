...
| **A link you share says what you were looking at, and nothing else.** | The address bar carries the mempool view (`?v=`), a block opened from the home ribbon (`?block=`) and a transaction you are tracking (`?tx=`) as plain query strings — so a link copied from a tracked transaction hands whoever opens it, and this site's own edge in that first request, the id of the transaction you were watching. That id already reaches this origin the moment you track it (`app/src/mempool/live-detail.ts` fetches `/api/xmr/tx/<id>`), and `Referrer-Policy: no-referrer` keeps it from any site you leave for. No other state is in a URL, and nothing about *you* is: the address describes the page, never the reader. Gated by `app/verify-tracking.mjs` §2b, §7 and §8. |
