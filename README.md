...
| **Usable at 390px, with a 12px type floor at phone widths.** | At 720px and below — the query is `max-width: 720px`, so 720 is inside it — `styles-legibility.css` lifts the label token to 12px, and `app/verify-mobile.mjs` measures the **rendered** result at 390 and 320 on every route, carrying no class exemption and a planted 9px positive control so a pass cannot be vacuous. SVG text is measured separately, in the space the reader is actually in, and one route's chart labels are **bounded rather than exempted** — a new offender fails the build. Above 720px there is no rendered floor at all. The label token clamps at 11px, but many classes declare smaller sizes outright: measured across all 18 routes at 1440, **232–256** HTML text nodes render below 11px (the range is the two feed states) and the smallest is **9px**. The provenance badges are the largest group among them, at 9.5px — the labels that say where a number came from are the smallest text on the desktop site. `app/verify-legibility.mjs` gates the **source** — the token scale, inline `fontSize` literals, SVG `fontSize` attributes — rather than what a selector finally resolves to. |
