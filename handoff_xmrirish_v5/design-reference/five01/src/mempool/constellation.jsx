...
function ConCard({ title, right, children, pad = "14px 16px", style }) {
  return (
...
function ConPropLog() {
  ...
  const fill = (s) => s.replace("{h}", 1 + (Math.random() * 9 | 0)).replace("{l}", 38 + (Math.random() * 90 | 0)).replace("{n}", 6 + (Math.random() * 8 | 0)).replace("{e}", 30 + (Math.random() * 12 | 0)).replace("{b}", 60 + (Math.random() * 9 | 0));
  ...
  const col = { p: "var(--p-50)", acc: "var(--tk-accent)", g: "var(--g-50)", dim: "var(--ink-60)" };
  return (
...
function ConVersionDonut() {
  const segs = [["0.18.4.0", 83.2, "var(--tk-accent)"], ["0.18.3.4", 14.1, "var(--y-50)"], ["older", 2.7, "var(--r-50)"]];
