...
const IS_TXID = (s) => /^[0-9a-f]{64}$/i.test(s.trim());
const IS_HEIGHT = (s) => /^\d{1,8}$/.test(s.trim());
...
function BlockBox({ block, confLabel, status, trackedHere, onClick, tone }) {
            ...
            <div style={{ fontSize: 9.5, lineHeight: 1.4, opacity: 0.92 }}>
              {isQueued ? (
                ...
                <>
                  ...
                  <div style={{ opacity: 0.75 }}>{Math.floor(block.age / 60)}m ago</div>
                </>
              )}
            </div>
          </>
...
function BlockRibbon({ data, tracking, onSelectBlock }) {
  ...
  const past = data.blocks.slice(0, 10);   // most recent = 1 conf, oldest = 10 conf
  ...
  past.forEach((b, i) => ribbon.push({ b, status: i === 0 ? "current" : "past", conf: b.conf, confLabel: b.conf + (b.conf === 1 ? " CONF" : " CONFS") }));
        ...
        {ribbon.map((r, i) => {
          ...
          return (
            <BlockBox
              ...
              trackedHere={isTracked ? (trackedConf + "/10") : null}
...
function TrackingStrip({ tracking, onClose }) {
  if (!tracking) {
    return (
...
function TxDetailPanel({ tx, onBack }) {
...
function ReactorConfirmationPanel({ tx }) {
...
function ReactorView({ data, bg }) {
  ...
  const onSearch = ({ kind, id, height }) => {
    if (kind === "tx") {
      const tx = lookupTx(id, data);
      setTracking({ kind: "tx", id, tx });
    } else if (kind === "block") {
  ...
  // Re-resolve tx each time data updates (confirmations grow)
  React.useEffect(() => {
    if (tracking?.kind === "tx") {
      setTracking((t) => ({ ...t, tx: lookupTx(t.id, data) }));
  ...
  return (
    <div className="art" data-screen-label="01 Mempool · REACTOR · v4 parity">
      ...
      <div className="art-stage">
        ...
        <div className="main" style={{ overflow: "auto", padding: 0 }}>
          {tracking?.kind === "tx" ? (
            ...
            <FullBlockDetail block={blockSynth(tracking.block, data)} onBack={clearTracking} onPickTx={(id) => onSearch({ kind: "tx", id })} />
          ) : (
...
function reactorThresholds(mempool) {
...
function ReactorCore({ mempool }) {
  const ref = React.useRef(null);
  const cx = 240, cy = 240, R = 220;
  // Pre-compute tier ring radii
  const tierR = { priority: 60, fast: 100, normal: 150, economy: 200 };
...
function ReactorBlockForge({ mempool, height }) {
...
function ReactorHashScope({ data }) {
...
function ReactorBlockPulse({ data }) {
  const [now, setNow] = React.useState(Date.now());
  React.useEffect(() => {
...
function ReactorTierRivers({ mempool }) {
  const thr = reactorThresholds(mempool);
  return (
    <div style={{ background: "rgba(0,0,0,0.45)", border: "1px solid var(--rule)", borderRadius: 8, padding: "14px 16px" }}>
...
function ReactorTxFeed({ mempool, onPickTx }) {
  const thr = reactorThresholds(mempool);
...
function OverviewPanels({ data, onPickTx }) {
...
function BlockDetailPanel({ block, onBack }) {
  return (
