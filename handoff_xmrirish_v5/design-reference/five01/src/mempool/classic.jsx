...
function classicLookupTx(txid, data) {
...
function ClassicSearch({ onSearch }) {
...
function ClassicBlock({ block, status, confLabel, trackedHere, onClick }) {
...
function ClassicRibbon({ data, tracking, onSelectBlock }) {
  const queued = [
  ...
  const ribbon = [
    ...
    ...past.map((b, i) => ({ b, status: i === 0 ? "current" : "past", confLabel: b.conf + (b.conf === 1 ? " confirmation" : " confirmations") })),
  ];
...
function ClassicConfirmationPanel({ tx, confChangedAt }) {
...
function ClassicKpi({ big, k, tone, fontSize = 28 }) {
  return (
    <div>
...
function ClassicTxDetail({ tx, onBack }) {
...
function ClassicView({ data, bg }) {
  const [tracking, setTracking] = React.useState(null);

  const onSearch = ({ kind, id, height }) => {
    if (kind === "tx") {
  ...
  React.useEffect(() => {
    if (tracking?.kind === "tx") {
      setTracking((t) => ({ ...t, tx: classicLookupTx(t.id, data) }));
    } else if (tracking?.kind === "block") {
      setTracking((t) => ({ ...t, block: data.blocks.find((b) => b.height === t.height) ?? t.block }));
    }
    // eslint-disable-next-line
  }, [data.height, data.mempool.length]);

  return (
    <div className="art" data-screen-label="01 Mempool · CLASSIC">
      {/* Calm, low-FX backdrop. No scanlines, lighter grid. */}
      <ArtBackground intensity="calm" scan={false} />
      <div className="art-stage">
        ...
        <ClassicRibbon data={data} tracking={tracking} onSelectBlock={(h) => onSearch({ kind: "block", height: h })} />
        ...
        <div className="main" style={{ overflow: "auto", padding: 0 }}>
          {tracking?.kind === "tx" ? (
            ...
            <FullBlockDetail block={blockSynth(tracking.block, data)} onBack={clear} onPickTx={(id) => onSearch({ kind: "tx", id })} />
          ) : (
            <ClassicLanding data={data} onPickTx={(id) => onSearch({ kind: "tx", id })} />
          )}
        </div>
...
function ClassicBlockDetail({ block, onBack }) {
  return (
...
function classicComputeThresholds(mempool) {
...
function classicBucketByTier(mempool) {
  const thr = classicComputeThresholds(mempool);
  const out = { priority: [], fast: [], normal: [], economy: [], thr };
...
function ClassicFeeHero({ buckets, xmrUsd }) {
  return (
    <section style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
      {CLASSIC_TIERS.map((tier, ti) => {
        ...
        const med = tx.length ? tx.map((t) => t.perB).sort((a, b) => a - b)[Math.floor(tx.length / 2)] : thrFallback;
...
function ClassicProjBlock({ buckets, mempool, height }) {
  const total = mempool.length || 1;
  const totalBytes = mempool.reduce((a, t) => a + t.size, 0);
...
function ClassicFeeDepth({ buckets }) {
  const total = CLASSIC_TIERS.reduce((a, tier) => a + buckets[tier.id].reduce((s, t) => s + t.size, 0), 0) || 1;
  return (
    <div style={{
      background: "rgba(0,0,0,0.45)", border: "1px solid var(--rule)",
      borderRadius: 8, padding: "14px 18px",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {CLASSIC_TIERS.map((tier) => {
          const tx = buckets[tier.id];
          const bytes = tx.reduce((s, t) => s + t.size, 0);
...
function ClassicTxFeed({ mempool, onPickTx, thr }) {
...
function ClassicChipRow({ data, mempool }) {
  const bytes = mempool.reduce((a, t) => a + t.size, 0);
  const avgFee = mempool.length ? mempool.reduce((a, t) => a + t.fee, 0) / mempool.length : 0;
...
function ClassicLanding({ data, onPickTx }) {
