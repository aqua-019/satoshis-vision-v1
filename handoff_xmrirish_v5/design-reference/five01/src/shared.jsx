...
function ParticleField({ density = 1.0, speed = 1.0, color = "rgba(255,122,26,0.35)", className }) {
  ...
  React.useEffect(() => {
    ...
    resize();
    // requestAnimationFrame breaks the synchronous loop that would
    // otherwise trigger "ResizeObserver loop completed" warnings.
    const ro = new ResizeObserver(() => requestAnimationFrame(resize));
    ro.observe(canvas);

    // Reduced particle density. The prior 120 + 8 streams produced cumulative
    // glow halos when ctx.shadowBlur stacked across many particles per frame.
    ...
    const streams = Array.from({ length: Math.floor(4 * density) }, () => ({
      x: Math.random() * w, y: h + Math.random() * h,
      vy: -(Math.random() * 1.2 + 0.5) * speed,
