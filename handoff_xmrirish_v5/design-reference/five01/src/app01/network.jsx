...
// + Tor/I2P share.
//
// All data sourced from `useMoneroLive()` except geo + version distribution which
// are augmented from static lookups here. Swap-in path documented in DATA.md.
...
function GeoMap({ width = 760, height = 380 }) {
  return (
    <svg viewBox="0 0 100 56" width="100%" style={{ display: "block", maxWidth: width, maxHeight: height }}>
      ...
      {(() => {
        ...
        const targets = PEER_GEO.map(([_c, _n, lon, lat]) => [(lon + 180) / 360 * 100, (75 - lat) / 135 * 100 * 0.56]);
        const lines = [];
        hubs.forEach((h, hi) => {
      ...
      {/* peers */}
      {PEER_GEO.map(([code, name, lon, lat, count, color], i) => {
        ...
        return (
          <g key={i}>
            ...
            <circle cx={x} cy={y} r={r * 1.5} fill="none" stroke={color} strokeWidth="0.15" opacity="0.6">
              <animate attributeName="r" values={`${r * 1.5};${r * 4};${r * 1.5}`} dur={(3 + i * 0.3) + "s"} repeatCount="indefinite" />
              <animate attributeName="opacity" values="0.6;0;0.6" dur={(3 + i * 0.3) + "s"} repeatCount="indefinite" />
            </circle>
            <circle cx={x} cy={y} r={r * 1.8} fill={color} opacity="0.14" />
            <circle cx={x} cy={y} r={r} fill={color} style={{ filter: `drop-shadow(0 0 0.5px ${color})` }}>
              <animate attributeName="opacity" values="0.95;0.5;0.95" dur={(2 + i * 0.2) + "s"} repeatCount="indefinite" />
            </circle>
...
function DifficultyCurve({ data, width = 560, height = 180 }) {
...
function BlockFullness({ data, width = 560, height = 180 }) {
...
function MempoolSizeOverTime({ data, width = 560, height = 180 }) {
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 12 }}>
        <PanelFrame title="Peer geography · live sample" right={<span>11 buckets · {PEER_GEO.reduce((a, p) => a + p[4], 0)} total</span>}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 6, marginTop: 10, fontSize: 10.5 }} className="mono">
            {PEER_GEO.map(([code, name, lon, lat, count, color]) => (
